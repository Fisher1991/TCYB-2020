...
class EIkMeans():
    ...
    def build_partition(self, data_train, test_size):
        ...
        while np.min(unique_count) < num_insts_part:
            ...
            C = kmeans.cluster_centers_
            num_k = np.unique(C_idx).shape[0]
            if num_k != C.shape[0]:
                kmeans = KMeans(n_clusters=num_k, n_init=1, random_state=0).fit(data_ini)
                C_idx = kmeans.labels_
                C = kmeans.cluster_centers_
            k_list, unique_count = np.unique(C_idx, return_counts=True)
            ...
            for _theta in self.theta:
                ...
                print("C_idx.shape[0]: ", C_idx.shape[0])
                if np.min(unique_count) > num_insts_part and C.shape[0] == unique_count.shape[0]:
                    break
    ...
    def drift_detection(self, data_test, alpha):
        ...
        k_list, unique_count = np.unique(C_idx, return_counts=True)
        print("k_list: ", k_list)
        print("k_list_astype: ", k_list.astype(int))
        print("unique_count: ", unique_count)

        observations[k_list.astype(int)] = unique_count
        print("observations: ", observations)
        observationstest = observations
        observationstest[C_idx[0]] = observationstest[C_idx[0]]+1
        print("observationstest: ", observationstest)
    ...
    def getHist(self, sample, initial_k_list, last_unique_count):
        C = self.C
        print("C: ", C)
        amplify_coe = self.amplify_coe
        print("Sample: ", sample)
        dist_mat = euclidean_distances(C, sample)
        C_idx = self.amplify_cluster(dist_mat, amplify_coe)
        print("C_idx:", C_idx)
        if last_unique_count == None:
            unique_count = np.zeros(initial_k_list.shape[0])
            print("unique_count:", unique_count)
            unique_count[C_idx[0]] = unique_count[C_idx[0]] + 1
        else:
            unique_count = last_unique_count
            unique_count[C_idx[0]] = unique_count[C_idx[0]] + 1
        return unique_count

    def drift_detection_new(self, initial_k_list, W_one_unique_count, W_two_unique_count, first_elment_in_W_two,
                                                                                                          sample, alpha):
        observations_one = np.zeros(initial_k_list.shape[0])
        observations_one[initial_k_list.astype(int)] = W_one_unique_count
        observations_two = np.zeros(initial_k_list.shape[0])
        observations_two[initial_k_list.astype(int)] = W_two_unique_count

        sample_dist_mat = euclidean_distances(self.C, sample)
        sample_C_idx = self.amplify_cluster(sample_dist_mat, self.amplify_coe)

        # 第一种情形
        observations_one_ca1 = observations_one
        observations_two_ca1 = observations_two
        first_elment_in_W_two_dist_mat = euclidean_distances(self.C, first_elment_in_W_two)
        first_elment_in_W_two_C_idx = self.amplify_cluster(first_elment_in_W_two_dist_mat, self.amplify_coe)
        observations_one_ca1[first_elment_in_W_two_C_idx[0]] = \
            observations_one_ca1[first_elment_in_W_two_C_idx[0]] + 1
        observations_two_ca1[first_elment_in_W_two_C_idx[0]] = observations_two_ca1[first_elment_in_W_two_C_idx[0]] - 1
        observations_two_ca1[sample_C_idx[0]] = observations_two_ca1[sample_C_idx[0]] + 1
        contingency_table_ca1 = np.array([observations_one_ca1, observations_two_ca1])
        ca1_chi2, ca1_p, ca1_dof, ca2_ex = chi2_contingency(contingency_table_ca1)

        # 第二种情形
        observations_one_ca2 = observations_one
        observations_two_ca2 = observations_two
        observations_two_ca2[sample_C_idx[0]] = observations_two_ca2[sample_C_idx[0]] + 1
        contingency_table_ca2 = np.array([observations_one, observations_two_ca2])
        ca2_chi2, ca2_p, ca2_dof, ca2_ex = chi2_contingency(contingency_table_ca2)
        if ca1_p <= ca2_p:
            best_location_type = 0
            updated_W_one_unique_count = observations_one_ca1
            updated_W_two_unique_count = observations_two_ca1
            if ca1_p < alpha:
                h = 1
            else:
                h = 0
        else:
            best_location_type = 1
            updated_W_one_unique_count = observations_one_ca2
            updated_W_two_unique_count = observations_two_ca2
            if ca2_p < alpha:
                h = 1
            else:
                h = 0

        return h, best_location_type, updated_W_one_unique_count, updated_W_two_unique_count

    def combineWindow(self, W_one_unique_count, W_two_unique_count):
        W_one_unique_count = W_one_unique_count + W_two_unique_count
        return W_one_unique_count
